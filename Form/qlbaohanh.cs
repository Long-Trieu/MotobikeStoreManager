...
namespace QLXM_8
{
    public partial class qlbaohanh : DevComponents.DotNetBar.Office2007RibbonForm
    {
        ...
        public bool CheckInfor(string mabh)
        {
            String chuoikn = "Provider=Microsoft.ACE.OLEDB.12.0;" +
                            "Data Source =" + Application.StartupPath + @"\qlxm.mdb;";
            objConnect = new OleDbConnection(chuoikn);
            // Tạo Command
            OleDbCommand cmd = new OleDbCommand("select * from Bao_Hanh where MaBH='" + mabh + "'", objConnect);
            objConnect.Open(); // Mở kết nối
        ...
        private void InsertByAdapter()
        {
            ...
            if (txt_stt.Text.Length == 0 || txt_mabh.Text.Length == 0 || cmb_makh.Text.Length == 0 || txt_sokhung.Text.Length == 0 || txt_somay.Text.Length == 0)
            {
                MessageBox.Show("Vui lòng nhập thông tin đầy đủ", "Lỗi");
                return;
            }
            if (CheckInfor(txt_mabh.Text))
            {
                MessageBox.Show("Mã bảo hành bị trùng", "Lỗi");
                return;
            }
